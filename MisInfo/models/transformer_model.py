...
from MisInfo.preprocessing.torch_dataloader import TorchDataset
...
class RobertaModel(Model):
    ...
    def train(self,
        ...
        train_data = TorchDataset(self.config, train_data)
        val_data = TorchDataset(self.config, val_data)
    ...
    def predict(self, data: List[Datum]) -> np.array:
        data = TorchDataset(self.config, data)
        dataloader = DataLoader(data,
                                batch_size=self.config["batch_size"],
                                pin_memory=True)
        self.model.eval()
        predicted = []
        #self.model.cuda() # deleting the .cuda()'s because not compatible with mac
        with torch.no_grad():
